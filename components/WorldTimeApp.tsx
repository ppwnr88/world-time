...
import AllTimezonesView from './AllTimezonesView'
...
function WorldTimeContent() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>('clocks')
  ...
  // Sync URL when clocks change (only in clocks view)
  useEffect(() => {
    if (!mounted || viewMode !== 'clocks' || activeClocks.length === 0) return
    const cityIds = activeClocks.map(c => c.id).join(',')
    const params = new URLSearchParams({ cities: cityIds })
    router.replace(`?${params.toString()}`, { scroll: false })
  }, [activeClocks, mounted, viewMode, router])
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/50 dark:from-gray-950 dark:via-slate-900 dark:to-indigo-950/30">
      ...
      <main className="flex-1 container mx-auto px-4 py-6 max-w-7xl">
        ...
        {/* ── ALL TIMEZONES VIEW ── */}
        {viewMode === 'all' && (
          <AllTimezonesView localTimezone={localTimezone} />
        )}
